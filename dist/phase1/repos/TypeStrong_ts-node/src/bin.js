...
function phase4(payload) {
    ...
    if (executeRepl) {
        ...
        const state = new repl_1.EvalState('');
        replStuff = {
            state,
            repl: (0, repl_1.createRepl)({
                state,
    ...
    if (replStuff)
        replStuff.state.path = (0, path_1.join)(cwd, (0, repl_1.REPL_FILENAME)(service.ts.version));
    if (isInChildProcess)
    ...
    if (executeEntrypoint) {
        if (payload.isInChildProcess && (0, util_2.versionGteLt)(process.versions.node, '18.6.0', '18.7.0')) {
...
function evalAndExitOnTsError(replService, module, code, isPrinted, filenameAndDirname) {
    ...
    if (isPrinted) {
        console.log(typeof result === 'string' ? result : (0, util_1.inspect)(result, { colors: process.stdout.isTTY }));
    }
}
