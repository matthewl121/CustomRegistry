...
exports.EvalState = exports.REPL_NAME = exports.STDIN_NAME = exports.STDIN_FILENAME = exports.EVAL_NAME = exports.EVAL_FILENAME = void 0;
exports.REPL_FILENAME = REPL_FILENAME;
...
/** @internal */
function REPL_FILENAME(tsVersion) {
    return (0, file_extensions_1.tsSupportsMtsCtsExts)(tsVersion) ? '<repl>.cts' : '<repl>.ts';
}
/** @internal */
...
function createRepl(options = {}) {
    ...
    const replService = {
        state: options.state ?? new EvalState((0, path_1.join)(process.cwd(), exports.EVAL_FILENAME)),
        setService,
        evalCode,
        evalCodeInternal,
        nodeEval,
    ...
    function setService(_service) {
        service = _service;
        if (mustSetStatePath)
            state.path = (0, path_1.join)(process.cwd(), REPL_FILENAME(service.ts.version));
    ...
    function declareExports() {
        if (declaredExports.has(context))
            return;
        ...
        function reset() {
            ...
            if (!service?.transpileOnly) {
                state.input += `// @ts-ignore\n${module_1.builtinModules
                    .filter((name) => !name.startsWith('_') && !name.includes('/') && !['console', 'module', 'process'].includes(name))
