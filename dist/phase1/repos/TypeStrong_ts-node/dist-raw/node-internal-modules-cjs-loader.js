...
const { cjsConditions, } = require('./node-internal-modules-cjs-helpers');
...
const { codes: { ERR_INVALID_MODULE_SPECIFIER, }, } = require('./node-internal-errors');
const { CHAR_FORWARD_SLASH, } = require('./node-internal-constants');
...
function createCjsLoader(opts) {
    ...
    const { encodedSepRegEx, packageExportsResolve, packageImportsResolve } = nodeEsmResolver;
    function tryPackage(requestPath, exts, isMain, originalPath) {
        ...
        if (actual === false) {
            ...
            if (!actual) {
                ...
                const jsonPath = path.resolve(requestPath, 'package.json');
                process.emitWarning(`Invalid 'main' field in '${jsonPath}' of '${pkg}'. ` +
    ...
    function statReplacementExtensions(p) {
        ...
        if (lastDotIndex >= 0) {
            ...
            if (ext === '.js' || ext === '.jsx' || ext === '.mjs' || ext === '.cjs') {
                ...
                const replacementExts = ext === '.js' ? replacementsForJs
...
//# sourceMappingURL=node-internal-modules-cjs-loader.js.map