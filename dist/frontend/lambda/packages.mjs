...
async function searchPackagesInS3(queries) {
    ...
    return packages.filter(pkg => queries.some(query => pkg.Name === query.Name && matchVersion(pkg.Version, query.Version)));
}
...
function getVersionType(version) {
    ...
    if (VALID_VERSIONS.EXACT.test(normalizedVersion))
        return "EXACT";
    if (VALID_VERSIONS.BOUNDED.test(normalizedVersion))
        return "BOUNDED";
    if (VALID_VERSIONS.CARAT.test(normalizedVersion))
        return "CARAT";
    if (VALID_VERSIONS.TILDE.test(normalizedVersion))
        return "TILDE";
    return null;
}
